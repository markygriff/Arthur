...
def get_weather_at_local():
    return owm.weather_at_place(LOCAL).get_weather()
...
def set_weather_details():
    ...
    DETAILS["clouds"] = WEATHER.get_clouds()
    DETAILS["rain"] = WEATHER.get_rain()
    DETAILS["snow"] = WEATHER.get_snow()
    DETAILS["wind"] = WEATHER.get_wind()
    DETAILS["humidity"] = WEATHER.get_humidity()
...
WEATHER = get_weather_at_local() # get a weather object for LOCAL
